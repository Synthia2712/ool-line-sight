...
const Timeline = () => {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 border-dotted">
        <div className="space-y-8">
          {phases.map((phase, phaseIndex) => <Card key={phaseIndex} className={`p-6 shadow-soft border-border/50 animate-fade-in-up transition-all hover:shadow-medium`} style={{
          animationDelay: `${phaseIndex * 100}ms`
              ...
              <div className="space-y-4 pl-4 border-l-2 border-border">
                {phase.milestones.map((milestone, milestoneIndex) => <div key={milestoneIndex} className="relative pl-6 pb-4 last:pb-0">
                    {/* Timeline dot */}
                    <div className={`absolute left-[-9px] top-1 h-4 w-4 rounded-full border-2 ${milestone.status === "complete" ? "bg-success border-success" : milestone.status === "in-progress" ? "bg-primary border-primary animate-pulse" : "bg-background border-border"}`} />
                    
                    <div className="flex items-center justify-between">
                      <div>
                        <p className={`font-medium ${milestone.status === "pending" ? "text-muted-foreground" : "text-foreground"}`}>
                          {milestone.name}
                        </p>
                      </div>
                      <Badge variant="outline" className={`${getStatusColor(milestone.status)} text-xs`}>
                        {milestone.status === "in-progress" ? "Active" : milestone.status === "complete" ? "Done" : "Pending"}
                      </Badge>
                    </div>
                  </div>)}
              </div>
